...
CREATE TRIGGER sortdate BEFORE INSERT On Borrow for each row
    begin
        if NEW.StartDate > NEW.DeadlineDate then
            SIGNAL SQLSTATE '50010' SET MESSAGE_TEXT = 'Invalid Dates';
        end if;
    end;
